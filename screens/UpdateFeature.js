...
const UpdateFeature = ({ route, navigation }) => {
  const [firstnameset, setValuefirstname] = useState(
    route.params.paramKey.feature
  );

  const [lastnameset, setValuelastname] = useState(route.params.paramKey.state);
  const {
    register,
    formState: { errors },
    handleSubmit,
  ...
  const editdetails = async (data) => {
    try {
      ...
      const response = await fetch(
        "http://localhost:8080/editrequest",
        requestOptions
      );
      ...
      if (response.ok) {
        ...
        // }
        alert("Feature Request is succussfully updated");
        // window.location.reload(false);

        navigation.dispatch(
          StackActions.replace("GetRequest", {
            user: "GetRequest",
          })
        );
        ...
        >
          <Box>
            <Text fontFamily={"bold"} bold fontSize={"2xl"}>
              EDIT DETAILS of request a feature
            </Text>
          </Box>
                  ...
                  <Box mt="9" mb="5" alignItems="center">
                    <FormControl>
                      ...
                      <input
                        ...
                        {...register("feature", { value: firstnameset })}
